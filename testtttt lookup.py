...
def merge_and_vlookup(file1, file2, output_file1, output_file2):
    try:
        ...
        # Create a new column 'KEY' by concatenating specified columns
       ...
       # Convert numeric columns to numeric data type
        ...
        merged_df1.loc[merged_df1.apply(lambda row: 'Addition in SR' if 'NA' in row.values else '', axis=1) != '', 'mismatched_reason'] = 'Addition in SR'
        ...
        merged_df2.loc[merged_df2.apply(lambda row: 'Addition in SR' if 'NA' in row.values else '', axis=1) != '', 'mismatched_reason'] = 'Addition in SR'
        ...
        merged_df1.loc[merged_df1['mismatched_reason'] == 'Addition in SR', 'Report Type'] = 'Addition in SR'
        ...
        merged_df2.loc[merged_df2['mismatched_reason'] == 'Addition in SR', 'Report Type'] = 'Addition in SR'
        ...
        # Create a DataFrame to store the report type counts for merged_df1
        report_counts_df_1 = pd.DataFrame(report_type_counts_1.items(), columns=['Report Type', 'Count'])

        # Write the report counts DataFrame to the first output file
        with pd.ExcelWriter(output_file1, mode='a', engine='openpyxl') as writer1:
            report_counts_df_1.to_excel(writer1, sheet_name='Sheet2', index=False, startcol=len(df1.columns)+2)

        # Count the occurrences of each type of report in merged_df2
        report_type_counts_2 = merged_df2['Report Type'].value_counts()

        # Create a DataFrame to store the report type counts for merged_df2
        report_counts_df_2 = pd.DataFrame(report_type_counts_2.items(), columns=['Report Type', 'Count'])

        # Write the report counts DataFrame to the second output file
        with pd.ExcelWriter(output_file2, mode='a', engine='openpyxl') as writer2:
            report_counts_df_2.to_excel(writer2, sheet_name='Sheet2', index=False, startcol=len(df2.columns)+2)
